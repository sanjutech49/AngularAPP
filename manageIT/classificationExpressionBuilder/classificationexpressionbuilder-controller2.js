manageitModule.controller("classificationExpressionBuilderController2", ['$scope', '$rootScope', 'newSharedScope', 'sharedScope', 'classificationService', 'classificationexpressionbuilderService',
    function ($scope, $rootScope, newSharedScope, sharedScope, classificationService, classificationexpressionbuilderService) {
        newSharedScope.store('classificationExpressionBuilderController2', $scope);
        ...
        $scope.getClassifications = function () {
            ...
            var contentImgDocLibId = "";
            if (contentType.contentTypeId)
            {
                contentImgDocLibId = contentType.contentTypeId;
            }
              
            else if (contentType.imageLibraryId)
            {
                contentImgDocLibId = contentType.imageLibraryId;
            }
            else if (contentType.documentLibraryId)
            {
                contentImgDocLibId = contentType.documentLibraryId;
            }
            $scope.classifications = [];
            classificationService.queryAll({ domainId: contentType.domainId, id: contentImgDocLibId }).$promise.then(function (details) {
                if (details) {
